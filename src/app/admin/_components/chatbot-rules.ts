...
// Shared text helpers, used by both this file and chatbot-suggesters.ts.
export function lc(s: string): string {
  return s.toLowerCase();
}

export function includes(haystack: string, needles: string[]): boolean {
    ...
    handler: () => ({
      parts: [
        { kind: "text", text: "I can help with:" },
        {
          kind: "bullets",
          items: [
            "SEO page title + meta description (try: 'suggest an SEO title for Goa')",
            "OG / share preview title + description (try: 'OG title')",
            "JSON-LD LocalBusiness schema generated from your settings",
            "Instagram captions + hashtag bundles per ceremony (try: 'caption for sangeet', 'hashtags for haldi')",
            "Inquiry response templates — three voices (try: 'respond to an inquiry')",
            "Title analyser — paste a title, get a 1–10 score (try: 'analyse title: ...')",
            "Site readiness check — flags missing fields before launch",
            "Looking up your services / packages / pricing line",
            "Listing valid gallery tags and field options",
            "Explaining any admin field ('what is X')",
            "Alt text + service blurb suggestions per ceremony",
            "How-to steps for every admin section",
            "Reading back current contact info, social handles, address",
          ],
        },
      ],
      suggestions: ["Suggest an SEO title", "Site readiness check", "Caption for sangeet", "Respond to an inquiry"],
    }),
  },
...
import { suggesterRules } from "./chatbot-suggesters";

// Combined rule list — base rules from this file plus the SEO/IG/JSON-LD
// /inquiry/analyser/checklist suggesters loaded from chatbot-suggesters.ts.
const allRules: Rule[] = [...rules, ...suggesterRules];

export function answer(query: string, ctx: Ctx): BotResponse {
  const q = lower(query.trim());
  if (!q) {
    return allRules.find((r) => r.id === "greeting")!.handler(q, ctx);
  }
  let best: { score: number; rule: Rule } = { score: 0, rule: allRules.find((r) => r.id === "fallback")! };
