...
// ──────────────────────────────────────────────────────────
function VariantHeroReels({
  onOpen,
  heroOverride,
  reelsOverride,
}: {
  onOpen: (f: Film) => void;
  heroOverride?: Partial<Film>;
  reelsOverride?: Film[];
}) {
  const hero: Film = { ...FILMS[0], ...heroOverride };
  const reels: Film[] = reelsOverride && reelsOverride.length > 0 ? reelsOverride : REELS;
  return (
...
export default function Films() {
  ...
  // Resolve hero + reels from Sanity-driven film list when present. Falls back
  // to the legacy featured-film fields if the films collection is empty.
  const sanityFeatures = site.filmList.filter((f) => f.kind === "feature");
  const sanityReels = site.filmList.filter((f) => f.kind === "reel");

  const heroOverride: Partial<Film> =
    sanityFeatures.length > 0
      ? {
          title: sanityFeatures[0].title,
          location: sanityFeatures[0].location,
          duration: sanityFeatures[0].duration,
          poster: sanityFeatures[0].poster,
          videoSrc: sanityFeatures[0].videoSrc,
        }
      : {
          // Fallback: use the legacy single featuredFilm shape from siteSettings.
          // (When Sanity has no `film` documents but featuredFilm is filled.)
          poster: site.filmPoster,
          videoSrc: site.filmVideoSrc,
        };

  // featuredFilm-based fallback for title/duration (Sanity featured film)
  if (sanityFeatures.length === 0) {
    heroOverride.title = site.films.featuredFilm.title;
    heroOverride.duration = site.films.featuredFilm.duration;
  }
  ...
  return (
    <section id="films" className="relative overflow-hidden bg-bg py-28 sm:py-36">
      <div className="mx-auto w-full max-w-7xl px-6 sm:px-10">
        <SectionLabel
          eyebrow={site.films.eyebrow}
          title={site.films.titleMain}
          script={site.films.titleScript}
        />
        ...
        >
          {site.films.description}
        </motion.p>

        <VariantHeroReels
          onOpen={setActiveFilm}
          heroOverride={heroOverride}
          reelsOverride={sanityReels.length > 0 ? sanityReels : undefined}
        />
